...
$(window).load(function() {
  $("#EDGE_CONNECT_PHASE").css("opacity", "1").hide().fadeIn(200, function() {
    $('#EDGE_CONNECT_PHASE').find(':input').not('select').filter(':visible:first').focus();
  ...
  if (jQuery.ui) {
      ...
      update: function (e, ui) {
        ...
        var $this = $(this),
          ...
          $accNr = $items.find("span[id*='_ACCNR']"),
          itemsOrder = [];
          $accNrOrder = [];
          ...
          $accNr.each(function (e) {
            $accVal = $accNr[e].innerHTML.substr($accNr[e].innerHTML.length - 5);
            $accNrOrder.push($accVal);
          });
        $("input[id*='_FAV_ACC']")[0].value = $accNrOrder;
        $(".SubmitFavorites").click(); 
        ...
        newCookie(thisID, itemsOrderStr, 365);
      },
      stop: function (e, ui) {
        ui.item.css('margin-top', '0px');
      },
...
$(window).load(function () {
  ...
  $containers.each(function () {
    ...
    if (readCookie(cookieName) != '') { // check if there is a cookie with custom order
      var elOrder = readCookie(cookieName).split(",");
      for (i = 0; i < elOrder.length; i++) {
        $('#' + elOrder[i]).appendTo($this); // arrange cards based on cookie
      }
    }
  });
